...
`startAttempt`/`runAttempt`/`finishAttempt`, `runSpan`, `measurePhase`, and `profileActivity` own attribution lifetimes. The Node runner profiles the actual `beforeEach`, test-body, and `afterEach` callbacks while retaining the default per-callback lifecycle timeouts and cleanup ordering. Declaration ownership is retained privately so helpers remain diagnostic callsites while their timing belongs to the discovered test file. `interrupt()` closes active spans and attempts, and `finish()` applies that interruption boundary before taking its snapshot. Descendant work that settles after its attempt is closed runs without attribution and increments `unattributedLateEventCount`. Custom activity labels are capped at 20 distinct values, after which new labels aggregate as `other`. Database and pool adapters pass only durations, booleans, bounded identifiers, numeric values, safe SQL operations, and precomputed opaque fingerprints through `recordDatabaseQuery`, `recordDatabaseTransaction`, and `recordPoolMetric`.
...
`mergeTestProfileTimingManifests(inputs)` and the merge command accept exactly one compatible profile for every shard. Every profile must use schema v1, represent an unfocused and unfiltered selection, match the same path base, discovered count, group count, and full file-set hash, and contain exactly its post-shard file count. A profile must have passed unless it is a supported empty group with `no-tests` status, zero selected files, an empty timing manifest, zero test/attempt counts, and no file or test records. Malformed, failed, interrupted, ordinary no-test, focused, filtered, duplicate, incomplete, colliding, or universe-mismatched inputs are rejected before `writeTimingManifest` atomically replaces the destination. Both `--output FILE` and `--output=FILE` forms are supported.
