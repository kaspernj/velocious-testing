...
Table declarations use `it.each(rows)(name, options?, callback)` and `describe.each(rows)(name, options?, callback)`. The rows value must be a non-empty, non-sparse array; this is validated before the returned declaration function or its name template is evaluated. Array rows spread; every other row is one argument. `%s`, `%d`, and `%j` consume positional row arguments, `%#` inserts the zero-based row index, `%%` inserts a percent sign, and `$path` reads an object-row property path. Unsupported tokens, missing positional values, invalid numeric/JSON values, non-object `$path` rows, and missing paths throw declaration errors. Duplicate generated names use the ordinary duplicate-declaration error.

Each builder captures its location immediately after validating rows, when `.each(rows)` is called. For `const cases = it.each(rows)` (also `test.each`) or `const suites = describe.each(rows)`, every generated row retains that construction location, even across later invocations of the same builder. Calling `runNodeTests({candidates: [file + ":" + builderLine]})` selects the generated tests or the generated suites' descendants, subject to the other selection filters. A later invocation line does not select the group by itself. Tests declared inside a generated suite retain their own child declaration lines and can still be selected there. Inline `.each(rows)(name, callback)` and options-plus-callback forms keep their existing behavior.

The locator contract returns a location object, including `{}` when no location is available. That same object is shared by every generated declaration and its declaration event; replacing the context locator after construction does not relocate an existing builder. An unused valid builder still captures once, and a throwing locator now throws at construction. Invalid rows fail before any capture. Name, template, and callback validation, row expansion, registration under the current parent, and suite callback execution remain deferred until invocation. Caller-owned row references and sequential async suite expansion are preserved. Ordinary declarations continue capturing their own locations. No protocol, schema, or event/result shape changes are involved.
