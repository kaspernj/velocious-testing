...
Argument, discovery, import, serialization, and writer failures are CLI failures rather than completed runs; they write a diagnostic to stderr and exit one without guaranteeing a JSON document. Help remains on stdout and exits zero. In a JSON CLI process, `console.log`, `console.info`, and `console.debug` route to the original stderr sink for the remainder of that process, including unawaited work and work scheduled during `beforeExit`. `console.warn` and `console.error` already use stderr. This process-lifetime ownership keeps stdout machine-readable without changing captured attempt output; reusable scoped routing still restores exact console descriptors when its scope ends.
