...
test("constructor implementations consistently use the selected class prototype and reset to neutral", () => {
  ...
  QueuedDouble.mockImplementation(Persistent)
  const later = new QueuedDouble("third")
  assert.equal(later instanceof Persistent, true)
  assert.equal(later instanceof QueuedDouble, true)
  assert.equal(Object.getPrototypeOf(later), Persistent.prototype)
  assert.equal(later.description(), "persistent:third")
  assert.equal(once instanceof QueuedDouble, true)
  assert.equal(fallback instanceof QueuedDouble, true)

  QueuedDouble.mockReset()
  assert.notEqual(QueuedDouble.prototype, Fallback.prototype)
  assert.notEqual(QueuedDouble.prototype, Persistent.prototype)
  const neutral = new QueuedDouble("ignored")
  assert.equal(neutral instanceof OneShot, false)
  assert.equal(neutral instanceof Fallback, false)
  assert.equal(neutral instanceof Persistent, false)
  assert.equal(neutral instanceof QueuedDouble, true)
  assert.equal(Object.getPrototypeOf(neutral), QueuedDouble.prototype)
