...
Only `Date`, `setTimeout`/`clearTimeout`, and `setInterval`/`clearInterval` are replaced. Advancing is synchronous and deterministically ordered; replacing an installation from its own callback stops that advancement before it can affect the new clock. `runPending()` snapshots each current occurrence once, and reentrant clock-control calls are rejected. Clock and due-time calculations outside the valid Date range throw without consuming pending work. Restoration discards pending work and reinstates the exact original properties. Retained setters from an older installation reject, while retained clearers cannot cancel a later installation's work. Compatible physical package copies share active-target ownership and the first captured real clocks, so they reject overlapping installation and cannot make a later-imported runner depend on fake time. See [Deterministic fake timers](docs/fake-timers.md) for scheduling and cleanup semantics.
...
Resource owners can end a shared run by throwing an error with `terminalResource: {scope: "run", name: "shared-resource"}`. The runner recognizes it through causes and aggregate members, suppresses retries/later callbacks, and cleans entered suites once. Remaining selected cases emit `test:not-run` and appear in `nonRunTests` with `status: "not-run"`, an originating `reason`, and a distinct `counts.notRun`; the run fails, including when terminal setup prevented every test body. Full causal stacks survive serialization and CLI reporting. See [Terminal resources and reporter migration](docs/terminal-resources.md) before enabling this contract in a resource owner or adapting results.

This includes errors from other realms and terminal throws/rejections from `testArgumentResolver`. Resolver failures complete terminal test/run reporting without entering the executor or per-test hooks. Ordinary resolver errors continue to reject `run()` with their original identity after suite cleanup.

Include tags require every requested tag by default; `includeTagMode: "any"` selects a test with at least one requested tag. `focusedTestsBypassIncludeTags: true` keeps focused tests eligible when includes do not match, while exclusions still win. A custom executor may set `attemptExecutorOwnsTimeout: true` to receive the declared `timeoutMs` without the kernel abandoning it at that deadline; that executor must apply its own timeout and settle only after its bounded cleanup. Empty suite names retain their legacy separators by default; adapters that use unnamed structural suites can set `omitEmptySuiteNames: true` to omit them consistently from full names, example matching, and reporting.

`suiteHookExecutor(input)` is a narrow downstream-adapter seam invoked once for each selected suite's `beforeAll` hook in declaration order and each `afterAll` hook in reverse declaration order. Its input is `{context, suite, hook, phase, timeoutMs, fullName, defaultExecute}`; `phase` is `"beforeAll"` or `"afterAll"`, and `fullName` is the nested suite name after the configured empty-name policy. Call `defaultExecute(args)` to invoke that hook with adapter-owned arguments. Omitting `args` is equivalent to `[]`, and the built-in executor calls `defaultExecute([])`, preserving zero-argument standalone behavior.

The package applies the suite timeout inside `defaultExecute`, owns hook ordering, treats executor or hook failures as failures of the supplied phase, exhausts reverse-order `afterAll` cleanup, and aggregates all cleanup failures. A `beforeAll` failure blocks the selected descendants; an `afterAll` failure marks the run failed. `TestRunner#cleanupActiveSuites()` and ordinary completion share the same cleanup promise, so active suites clean up inside-out and each `afterAll` runs exactly once. `runNodeTests()` forwards the same option. Framework adapters can add arguments and wrap execution without taking over traversal or cleanup:

```js
const result = await runTests({
  context,
  suiteHookExecutor: async ({phase, fullName, defaultExecute}) => {
    await profiler.span(`${phase}:${fullName}`, async () => {
      await defaultExecute([{configuration}])
    })
  }
})
```
