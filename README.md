...
`it.each(rows)` and `describe.each(rows)` require a non-empty, non-sparse rows array, spread array rows into positional callback arguments, and pass scalar or object rows as one argument. Names support `%%`, `%#`, `%s`, `%d`, `%j`, and `$path` for object rows. Generated declarations keep the line where `.each(rows)` creates the builder for path-line selection and diagnostics, including when that builder is stored and invoked later or reused:

```js
const cases = it.each([[1, 2, 3], [2, 3, 5]]) // Select this builder line.

describe("calculator", () => {
  cases("adds row %#", (left, right, sum) => {
    expect(left + right).toEqual(sum)
  })
})
```

`runNodeTests({candidates: [file + ":" + builderLine]})` selects every test generated by that builder, subject to the other selection filters. Selecting a stored `describe.each` builder line selects its suites' descendant tests; those tests retain their own declaration locations. Registration still happens when the returned builder is invoked. See [Declaration states and tables](docs/declaration-state.md) for capture and validation timing.
...
Return or await promise assertions so the runner can apply its normal timeout, retry, and failure accounting. Recursive containing patterns and their failure diagnostics are cycle-safe, and promise-like custom matcher results read `then` once while preserving its receiver. For legacy facade compatibility, `toEqual` uses coercive comparison for top-level primitive pairs and for a top-level dense singleton primitive array compared with a primitive in either direction; `toHaveAttributes` does the same for each returned primitive attribute. Nested object and array equality remains strict, and functions and arbitrary objects are never coerced. Chained `toChange`/`andChange` probes run sequentially before the action and sequentially afterward. Positive equality and mock-argument failures include bounded, deterministic structural differences. See [Advanced matchers](docs/matchers.md) for settlement behavior, custom matcher validation and TypeScript augmentation.

Framework compatibility facades can also import `isArrayContaining`, `isObjectContaining`, `matchArrayContaining`, and `matchObject` from the browser-safe root. The predicates recognize only package-branded asymmetric values. The match helpers return `{matches, differences}` while delegating comparison to the same canonical equality engine used by `expect`; applications should normally prefer the high-level expectation API.
