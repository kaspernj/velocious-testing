...
Failures and an empty selection exit nonzero. `--retries COUNT` and `--timeout MS` set run defaults; declarations may use `retries`, `timeoutMs`, or `timeoutSeconds`. Tag, example/name, retry/retries, setup, and reporter aliases accept both space and equals syntax. Paired `--groups`/`--group-number` options create deterministic, complete, disjoint, 1-indexed partitions; a validated timing manifest supplies weights and otherwise the documented path-based fallback weights apply. Profile output uses the `velocious.test-profile` v1 schema, is written atomically, and can be strictly aggregated into a portable timing manifest only when every compatible shard completed cleanly. See [Standalone CLI, discovery, grouping, and timing](docs/testing-cli.md) and [Test profiling](docs/test-profiling.md).
